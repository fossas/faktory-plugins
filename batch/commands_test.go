...
func TestBatchOpenAfterCallbackStarted(t *testing.T) {
	withServer(func(s *server.Server, cl *client.Client) {
		...
		// Commit batch (empty batch = callbacks fire immediately)
		_, err = cl.Generic("BATCH COMMIT " + bid)
		require.NoError(t, err)

		// Wait a moment for callback to fire
		// Since the batch has no jobs, the success callback should be enqueued immediately

		// Manually set the callback state to simulate callback started
		redis := s.Manager().Redis()
		redis.Set(ctx, batchSuccessStateKey(bid), CallbackEnqueued, 0)

		// Try to open batch after callback started
